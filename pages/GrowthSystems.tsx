  ...
  return (
    <div className="page-bg" style={{fontFamily:"'DM Sans',system-ui,sans-serif"}}>
        ...
        <div style={{maxWidth:1100,margin:'0 auto',padding:'0 28px',position:'relative',zIndex:2}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:64,alignItems:'center'}} className="hero-grid-cols">
            <div>
              ...
              <h1 className="reveal d1" style={{fontFamily:"'Instrument Serif',Georgia,serif",fontSize:'clamp(40px,6vw,78px)',fontWeight:400,letterSpacing:'-2px',lineHeight:1.02,marginBottom:20,color:'rgba(255,255,255,0.96)'}}>
              ...
              <p className="reveal d2" style={{fontSize:16,fontWeight:300,color:'rgba(255,255,255,0.5)',lineHeight:1.72,marginBottom:36,maxWidth:460}}>
                70% of leads are lost due to slow response times. Deploy intelligent agents that qualify, nurture, book, and close — around the clock.
              </p>
            ...
            <div className="reveal-r d2">
              <div className="glass-card" style={{borderRadius:22,padding:28,animation:'float 9s ease-in-out infinite'}}>
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:22,paddingBottom:18,borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
                  <div style={{display:'flex',alignItems:'center',gap:10}}><Bot size={20} color="#4f9eff"/><span style={{fontWeight:500,color:'rgba(255,255,255,0.88)',fontSize:15}}>AI Sales Agent</span></div>
                ...
                {[['Leads Processed Today','142','rgba(255,255,255,0.72)'],['Meetings Booked','18','#4f9eff'],['Avg Response Time','0.8s','#34d399'],['Conversion Rate','12.7%','#818cf8']].map(([l,v,c])=>(
      ...
      <div style={{maxWidth:1100,margin:'0 auto',padding:'88px 28px',position:'relative',zIndex:1}}>
        ...
        <h2 className="reveal d1" style={{fontFamily:"'Instrument Serif',Georgia,serif",fontSize:'clamp(28px,4vw,48px)',fontWeight:400,letterSpacing:'-1px',textAlign:'center',marginBottom:56,color:'rgba(255,255,255,0.95)',lineHeight:1.1}}>Four systems that <em>generate revenue</em> while you sleep.</h2>
        <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:14}} className="two-cols">
          {[
            {icon:'🤖',color:'#4f9eff',title:'AI Lead Qualification',desc:'Intelligent chatbots pre-qualify every inbound lead using custom scoring logic — filtering out noise so your sales team only speaks to buyers.',feats:['24/7 automated qualification','Custom scoring criteria','CRM sync in real-time','Multi-channel deployment']},
      ...
      <div style={{maxWidth:1100,margin:'0 auto',padding:'0 28px 88px',position:'relative',zIndex:1}}>
        <div className="reveal" style={{background:'rgba(8,14,26,0.7)',backdropFilter:'blur(60px)',border:'1px solid rgba(255,255,255,0.09)',borderRadius:28,padding:'80px 48px',textAlign:'center',position:'relative',overflow:'hidden'}}>
          ...
          <h2 style={{fontFamily:"'Instrument Serif',Georgia,serif",fontSize:'clamp(28px,4.5vw,52px)',fontWeight:400,letterSpacing:'-1.5px',lineHeight:1.06,marginBottom:14,color:'rgba(255,255,255,0.96)'}}>
            Ready to build your <em style={{color:'#4f9eff'}}>AI workforce?</em>
          </h2>
      ...
      <style>{`@keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-12px)}} @media(max-width:900px){.hero-grid-cols,.two-cols{grid-template-columns:1fr!important;gap:32px!important;}}`}</style>
    </div>
  );
};
