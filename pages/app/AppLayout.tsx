  ...
  return (
    <>
      <nav id="tabbar">
        ...
        <button className={on('nutrition') ? 'on' : ''} onClick={() => navigate('/app')}>
          <Icon name="nutrition" />
          <span>Nutrition</span>
        </button>
        ...
        <button className={on('stats') ? 'on' : ''} onClick={() => navigate('/app/stats')}>
          <Icon name="chart" />
          <span>Progress</span>
        </button>

        <button className={on('library') ? 'on' : ''} onClick={() => navigate('/app/library')}>
          <Icon name="list" />
          <span>Library</span>
        </button>
      </nav>
  ...
  return (
    <aside className="hidden lg:flex flex-col w-56 h-screen fixed left-0 top-0 bg-[#07090e] border-r border-[#172744] z-40 py-8 px-4">
      ...
      <nav className="space-y-1 flex-1">
        {[
          { path: '/app', label: 'Home', icon: 'house', exact: true },
          { path: '/app/workout', label: 'Workout', icon: 'dumbbell', exact: false },
          { path: '/app/library', label: 'Exercise Library', icon: 'list', exact: false },
          { path: '/app/stats', label: 'Progress & Stats', icon: 'chart', exact: false },
          { path: '/app/leaderboard', label: 'Leaderboard', icon: 'trophy', exact: false },
          { path: '/app/profile', label: 'Profile & Settings', icon: 'gear', exact: false },
        ].map(n => (
          <NavLink
            key={n.path}
            to={n.path}
